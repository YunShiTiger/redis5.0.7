 ...
 * 用于存储整数有序集合的一种实现方式
 *    需要注意元素是唯一的  且大小是有顺序的
 * 本整数集合的核心在于能够根据整数的编码字节数来存储整数 同时在遇到整数编码不能满足时进行编码方式升级处理  注意只升不降
 */
 ...
 * the value is not present in the intset and sets "pos" to the position where "value" can be inserted. */
...
static intset *intsetUpgradeAndAdd(intset *is, int64_t value) {
    ...
    //循环处理原始整数集合中的元素占据的空间------>这个地方设置的非常的巧妙 
    //使用一个标记就完成了给对应的位置留出空间方便后期进行插入元素的处理
    while(length--)
...
intset *intsetAdd(intset *is, int64_t value, uint8_t *success) {
    ...
    if (valenc > intrev32ifbe(is->encoding)) {
		...
		//在没有存在的前提下 进行整数集合的空间扩充处理---->此处的核心是进行腾出对应的空间
        is = intsetResize(is,intrev32ifbe(is->length)+1);
