 ...
 * 首先在Redis中Hash对象的底层实现方式有两种 一种是压缩列表 一种是字典结构
 * 因此，在一定的情况下可能需要进行类型实现的转换 即 将压缩列表的形式 转换成 字典结构的形式
 * 需要注意 只能进行向上变化类型 不能向下进行变化操作处理
 */
...
/* Check the length of a number of objects to see if we need to convert a ziplist to a real hash. 
...
/* Get the value from a ziplist encoded hash, identified by field. Returns -1 when the field cannot be found. */
 ...
 * can always check the function return by checking the return value for C_OK and checking if vll (or vstr) is NULL. */
 ...
 * The function returns NULL if the field is not found in the hash. Otherwise a newly allocated string object with the value is returned. */
/* 根据给定的字段获取对应的值对象 */
...
/* Add a new field, overwrite the old with the new value if it already exists. Return 0 on insert and 1 on update.
 *
 ...
 *
 * HASH_SET_COPY corresponds to no flags passed, and means the default semantics of copying the values if needed. 
 */
#define HASH_SET_TAKE_FIELD (1<<0)
#define HASH_SET_TAKE_VALUE (1<<1)
#define HASH_SET_COPY 0
/* 在hash对象中插入或者覆盖对应字段和值，插入返回0 更新返回1 */
    ...
    if (o->encoding == OBJ_ENCODING_ZIPLIST) {
        ...
        if (fptr != NULL) {
            ...
            if (fptr != NULL) {
                ...
                /* Delete value */
				//进行删除对应的原始值节点内容 注意删除元素可能导致压缩列表的抖动
                zl = ziplistDelete(zl, &vptr);

                /* Insert new value */
				//将新的值节点内容设置到ziplist中 注意添加元素可能导致压缩列表的抖动
        ...
        if (hashTypeLength(o) > server.hash_max_ziplist_entries)
     	...
     	//在字典结构中查询是否有对应字段的信息结构节点
        dictEntry *de = dictFind(o->ptr,field);
		//检测在字典结构中是否找到此对应的信息节点
        if (de) {
			//释放在字典结构中字段对应的原始值字符串占据的空间
...
int hashTypeDelete(robj *o, sds field) {
    ...
    if (o->encoding == OBJ_ENCODING_ZIPLIST) {
    	...
    	//尝试在对应的字典结构中删除对应的字段和值
        if (dictDelete((dict*)o->ptr, field) == C_OK) {
			//设置删除标识
            deleted = 1;
            /* Always check if the dictionary needs a resize after a delete. */
			//删除一个元素后,检测是否需要进行重新设置空间分配处理 即元素个数不足槽位数的10% 进行缩容处理 减少对应的槽位数
...
unsigned long hashTypeLength(const robj *o) {
	...
	//根据hash对象的不同实现来获取对应的元素数量值
    if (o->encoding == OBJ_ENCODING_ZIPLIST) {
		//获取hash对象对应的压缩列表实现类型对应的元素数量值 此处除以2 是分开键值对
        length = ziplistLen(o->ptr) / 2;
    } else if (o->encoding == OBJ_ENCODING_HT) {
    	//获取hash对象对应的字典实现类型对应的元素数量值
...
void hashTypeReleaseIterator(hashTypeIterator *hi) {
    if (hi->encoding == OBJ_ENCODING_HT)
		//如果hash对象是字典实现类型,这个地方需要释放对应的字典的迭代器占据的空间
...
/* Get the field or value at iterator cursor, for an iterator on a hash value encoded as a ziplist. Prototype is similar to `hashTypeGetFromZiplist`. */
...
/* 根据提供的迭代器状态从对应的字典结构中获取对应的当前需要遍历的节点信息 */
 ...
 * a number.
 *
 * If *vll is populated *vstr is set to NULL, so the caller can always check the function return by checking the return value type checking if vstr == NULL. */
...
/* 检测对应键所对应的hash对象是否在redis字典数据库中, 没有就进行创建操作处理,类型不对向客户端进行错误类型响应 */
robj *hashTypeLookupWriteOrCreate(client *c, robj *key) {
	...
	//返回找到或者新建的hash对象 即完成准备好hash对象的处理
    return o;
}

/* 实现将ziplist结构数据转化成对应的字典结构类型 */
void hashTypeConvertZiplist(robj *o, int enc) {
    ...
    if (enc == OBJ_ENCODING_ZIPLIST) {
		...
		//循环遍历老数据,将ziplist结构中的数据转化成字典结构上的数据
...
/* 将对应的ziplist结构的hash对象转换成字典结构的hash对象 */
void hashTypeConvert(robj *o, int enc) {
    if (o->encoding == OBJ_ENCODING_ZIPLIST) {
		//实现从ziplist转换成字典类型的结构变化
...
void hsetnxCommand(client *c) {
	//定义用于存储hash对象的指针
    robj *o;
	//检测键所对应的hash对象是否存在并进行创建操作处理--->即在对应的redis数据库字典中查询是否有对应的键值对对象 如果没有就创建对应的键对象
    if ((o = hashTypeLookupWriteOrCreate(c,c->argv[1])) == NULL) 
		return;
	//检测新引入的字段和值的内容是否引起本hash对象底层实现的变换----->即ziplist结构转换成字典结构
...
void hsetCommand(client *c) {
	...
	//检测客户端对应的传入参数是否是对应的奇数个参数 一个键对象加对应的n组字段和值数据
    if ((c->argc % 2) == 1) {
 ...
 *
 *   通过分析对应的处理过程,发现在这个命令中出现了两次进行遍历处理(如果是ziplist的话,效率会比较低,所以数据元素多了的话,需要升级为字典结构)
 *         1 首先查找对应字段所对应的老值
 *         2 将新值再次插入到hash对象中
 */
...
void hdelCommand(client *c) {
    ...
    if (deleted) {
		...
		//增加脏数据计数值 这个地方删除了一个键对象中的多个字段 也算是多次脏数据处理
...
static void addHashIteratorCursorToReply(client *c, hashTypeIterator *hi, int what) {
    ...
    if (hi->encoding == OBJ_ENCODING_ZIPLIST) {
		...
		//将字符串类型数据填充到响应集合中-------->这个地方字典结构中并没有进行对字符串数据进行整数编码处理操作
        addReplyBulkCBuffer(c, value, sdslen(value));
    } else {
        serverPanic("Unknown hash encoding");
    }
...
void genericHgetallCommand(client *c, int flags) {
	...
	//创建对应的迭代器对象----->注意对于字典来说 这是一个普通的迭代器
    hi = hashTypeInitIterator(o);
	//循环遍历hash对象中所有的元素
    while (hashTypeNext(hi) != C_ERR) {
		/获取迭代上对应的字段信息
